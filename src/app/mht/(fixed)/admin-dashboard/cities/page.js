...
import { Grid, Box, Typography, Link, Divider, Button, Card, CardMedia, CardActions, CardContent, InputAdornment, TextField } from '@mui/material';
...
export default function Management() {
    ...
    const goToEditCity = (city) => {
        router.push(`/edit/${city.state.toLowerCase()}/${city.name.toLowerCase().replaceAll(/\s/g, "-")}`);
    };
    ...
    if (!hasLoaded) {
        return (
            <Box display='flex' justifyContent='center' sx={{height:'100vh', padding: '5em'}}>
                <Loading size={50}/>
            </Box>
        );
    }
    ...
    return (
        <>
            <AskYesNoDialog {...confirmDeleteCityProps} open={showConfirmDeleteCity} />
            <AddEditCityDialog
            ...
            <Grid container item sm={12} display='flex' sx={{position:"relative"}}>
                ...
                <Box marginBottom={4} width="100%">
                    ...
                    <Typography component='div' fontWeight={700} variant={'h3'} align={'center'} gutterBottom>
                ...
                {cities.length === 0 && hasLoaded && 
                    <>
                        ...
                        <Typography variant={'subtitle1'} component={'p'} color={'textSecondary'} align={'center'} gutterBottom>
                            You don&apos;t have any cities assigned to your account.
                        </Typography>
                        <Typography variant={'subtitle2'} align={'center'}>
                            Expected something different?{' '}
                            <Link component={'a'} color={'primary'} href={"mailto:admin-representative@myhometown.org"} underline={'none'}>
                ...
                {cities.length === 1 &&
                    <Box marginBottom={4} width="100%">
                        <SingleCity city={cities[0]} goToEditCity={goToEditCity} handleEditCity = {handleEditCity} />
                    </Box>
                }
                ...
                <RoleGuard requiredRole='admin' user={user}>
                    <Box display='flex' justifyContent='center' width='100%' m = {3}>
                        <Button variant="contained" color="primary" onClick={() => setShowAddCityForm(true)}>
                            Add a City
                        </Button>
                    </Box>
                </RoleGuard> 
...
function AddCityStatsForm({ city, updateCityStats }) {
    ...
    const validate = () => {
        ...
        Object.keys(cityStats).forEach(key => {
            if (!cityStats[key] || isNaN(cityStats[key]) || Number(cityStats[key]) <= 0) {
    ...
    return (
        <Box sx={{ p: 4 }} component="form" onSubmit={handleSubmit}>
            <Typography variant="h5" textAlign="center">Total for 2024</Typography>
            <Typography variant="subtitle1" textAlign="center">Please update these values at the end of every month.</Typography>
...
export function SingleCity({ city, goToEditCity, handleEditCity }) {
    ...
    return (
        <Grid container direction="column" alignItems="center" display='flex' spacing={2}> 
                ...
                >
                    ...
                    >
                        ...
                        <CardActions sx={{ justifyContent: 'flex-end' }}>
                            <Button size="small" href={`/edit/${city.state.toLowerCase()}/${city.name.toLowerCase().replaceAll(/\s/g, "-")}`}>
                                Edit
                            </Button>
                        </CardActions>
                    </Card>
