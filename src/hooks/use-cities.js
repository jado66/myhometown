...
export default function useCities(email = null) {
    ...
    useEffect(() => {
        ...
        let finalCities = storedCities || mockCities;

        if (email) {
            finalCities = finalCities.filter(city =>
                city.cityOwners.some(owner => owner.email.toLowerCase() === email.toLowerCase())
            );
        } 
    
        setCities(finalCities);

        if (!storedCities && !email) {
    ...
    useEffect(() => {
        ...
        const newSelectOptions = cities.map(city => ({
            value: city.id,
            label: city.name // or any other user property that you want to display in the dropdown
        }));
    ...
    const handleAddCity = (city) => {
        ...
        const storedCities = JSON.parse(localStorage.getItem('cities'));
        if (storedCities) {
            localStorage.setItem('cities', JSON.stringify([...storedCities, city]));
        }
        else {
    ...
    const handleEditCity = (id, city) => {
        setCities(cities.map((c) => (c.id === id ? city : c)));

        const storedCities = JSON.parse(localStorage.getItem('cities'));
        if (storedCities) {
            localStorage.setItem('cities', JSON.stringify(storedCities.map((c) => (c.id === id ? city : c))))
        }
        else {
            localStorage.setItem('cities', JSON.stringify(cities.map((c) => (c.id === id ? city : c))))
        }
    };
    ...
    const handleDeleteCity = (id) => {
        ...
        if (storedCities) {
            localStorage.setItem('cities', JSON.stringify(storedCities.filter((c) => c.id !== id)));
        }
        else {
            localStorage.setItem('cities', JSON.stringify(cities.filter((c) => c.id !== id)));
        }
    };
